// Parsing and PPS ISR (No Timer checking)
...
volatile uint8_t  gps_pps_present = 0;
...
void PPS_Init(void)
{
    ...
    GPIOPadConfigSet(PPS_PORT_BASE, PPS_PIN, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if (ubx_message_ready)
        {
            ...
            if (ubx_class == 0x01 && ubx_id == 0x20 && ubx_len == 16)
            {
                    ...
                    timegps_data.tAcc < 50000)
                {
                    ...
                    uint32_t ubx_abs_sec = (timegps_data.week * 604800) + tow_sec;

                    if ((timegps_data.iTOW % 1000) < 10 || (timegps_data.iTOW % 1000) > 990)
                    {
                        IntMasterDisable();
        ...
        /* PPS timeout detection (simple loop based) */
        static uint32_t pps_timeout_counter = 0;

        if (gps_pps_present)
        {
            gps_pps_present = 0;
            pps_timeout_counter = 0;
            pps_missed = 0;
        }
        else
        {
            if (++pps_timeout_counter > 200000)
                pps_missed = 1;
        }

        /* gps_system_seconds is now your KAVACH synchronized time */
    }
}
